...
# ---- TEXT (Paysafe + allgemeine Weiterleitung) ----
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if paysafe_pattern.match(text):
        ...
        try:
            ...
            print(f"❌ Weiterleiten fehlgeschlagen: {e}")
            try:
                fallback = (
                    f"💬 Nachricht von @{from_user.username or from_user.first_name} (ID: {user_id}):\n\n"
                    f"{text}"
                )
                sent = await context.bot.send_message(chat_id=ADMIN_CHAT_ID, text=fallback)
                forwarded_msg_to_user[sent.message_id] = user_id
