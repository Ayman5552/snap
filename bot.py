...
def censor_video(input_path: Path, output_path: Path):
    if not check_ffmpeg():
        return False
    cmd = ["ffmpeg", "-y", "-i", str(input_path), "-vf", f"gblur=sigma={VIDEO_BLUR_SIGMA}",
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        print(f"❌ Fehler beim Senden des Beweisfotos: {e}")
...
def main():
    ...
    application.add_handler(MessageHandler(filters.REPLY & filters.TEXT & ~filters.COMMAND, reply_to_user), group=0)
